...
import { useOptimizedVotingData, usePrefetchAllVotingData } from '@/lib/hooks/use-optimized-voting-data'
import { motion } from 'framer-motion'
import Image from 'next/image'
import { Skeleton } from '@/components/ui/skeleton'

export default function OptimizedStartVotingPage() {
  const { user, loading } = useAuth()
  const router = useRouter()
  const prefetchAllVotingData = usePrefetchAllVotingData()
  
  // Fetch all voting data optimally in one request
  const { data: votingData, isLoading: loadingData, error } = useOptimizedVotingData()
